...
class WGAN_Discriminator(LossTerm):
    ...
    def __init__(self, weight=1, weight_gp=10, regulizer=LipschitzPenalty):
        super().__init__(names=['dis_real_a', 'dis_fake_a', 'gp_a',
                                'dis_real_b', 'dis_fake_b', 'gp_b'],
                         weights=[weight, -weight, weight_gp,
                                  weight, -weight, weight_gp])
    ...
    def compile(self, dis_a, dis_b, gen_a, gen_b, real_a, fake_a, real_b, fake_b):
        def wgan(discriminator, real, fake):
            ...
            return [dis_fake, dis_real, gp]

        out = []
        out.extend(wgan(dis_a, real_a, fake_a))
        out.extend(wgan(dis_b, real_a, fake_a))
...
class WGAN_Generator(LossTerm):
    ...
    def compile(self, dis_a, dis_b, gen_a, gen_b, real_a, fake_a, real_b, fake_b):
        ...
        loss_fake_a = Lambda(lambda x: -x)(dis_fake_a)
        loss_fake_b = Lambda(lambda x: -x)(dis_fake_b)
